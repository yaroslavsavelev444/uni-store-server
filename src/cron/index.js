...
class CronService {
   ...
   */
  initialize() {
    info("[CRON] Инициализация крон-сервиса");
    ...
    try {
      ...
      info(`[CRON] Запланировано ${this.jobs.size} заданий`);
    } catch (error) {
      _error("[CRON] Ошибка при инициализации:", error);
      throw error;
    }
  }
  ...
  scheduleJob(name, schedule, task) {
    if (this.jobs.has(name)) {
      warn(`[CRON] Задание "${name}" уже существует, заменяем`);
    ...
    info(`[CRON] Задание "${name}" запланировано: ${schedule}`);
  }
  ...
  async executeTask(name, task) {
    ...
    info(`[CRON:${name}] Начало выполнения #${executionId}`);
    ...
    try {
      ...
      info(`[CRON:${name}] Успешно выполнено #${executionId} за ${duration}ms`);
    } catch (error) {
      this.stats.totalExecutions++;
      this.stats.failedExecutions++;

      _error(`[CRON:${name}] Ошибка при выполнении #${executionId}:`, {
  ...
  async cleanupTempFiles() {
    ...
    const config = {
      maxAgeHours: parseInt(process.env.TEMP_FILE_MAX_AGE_HOURS, 10) || 24,
      protectedFiles: ["readme.txt", ".gitkeep"],
      dryRun: process.env.CRON_DRY_RUN === "true",
    };

    info("[CRON:TEMP_CLEANUP] Начало очистки временных файлов", config);
    ...
    try {
      ...
      try {
        ...
        info("[CRON:TEMP_CLEANUP] Директория temp не существует, пропускаем");
        return;
      }
      ...
      for (const file of files) {
        ...
        if (config.protectedFiles.includes(file)) {
          debug(`[CRON:TEMP_CLEANUP] Пропущен защищенный файл: ${file}`);
          skippedCount++;
          continue;
        }
        ...
        try {
          ...
          errorCount++;
          warn(
            `[CRON:TEMP_CLEANUP] Ошибка при обработке файла ${file}:`,
            error.message,
          );
        }
      }
      ...
      if (deletedCount > 0 || errorCount > 0) {
        info("[CRON:TEMP_CLEANUP] Результаты очистки:", stats);
      } else {
        debug("[CRON:TEMP_CLEANUP] Нечего удалять", stats);
      }
    } catch (error) {
      _error("[CRON:TEMP_CLEANUP] Критическая ошибка:", error);
      throw error;
    }
  }
  ...
  async cleanupOldFeedbacks() {
    ...
    try {
      ...
      const retentionDays =
        parseInt(process.env.FEEDBACK_RETENTION_DAYS, 10) || 365;
      ...
      if (oldFeedbacks.length === 0) {
        info("[CRON:FEEDBACK_CLEANUP] Старых фидбеков не найдено");
        return;
      }
      ...
      info("[CRON:FEEDBACK_CLEANUP] Удалено фидбеков:", {
        count: deleteResult.deletedCount,
        fileCount: filesToDelete.length,
        retentionDays,
      });
    } catch (error) {
      _error("[CRON:FEEDBACK_CLEANUP] Ошибка:", error);
      throw error;
    }
  }
  ...
  async deleteFeedbackFiles(fileNames) {
    ...
    for (const fileName of fileNames) {
      ...
      try {
        ...
        await fs.unlink(filePath);
        deletedCount++;
        debug(`[CRON] Удален файл фидбека: ${fileName}`);
      } catch (error) {
        if (error.code === "ENOENT") {
          debug(`[CRON] Файл уже удален: ${fileName}`);
        } else {
          errorCount++;
          warn(`[CRON] Ошибка при удалении файла ${fileName}:`, error.message);
        }
      }
    }
  ...
  async monitorDiskSpace() {
    try {
      ...
      // Проверяем лимиты
      const maxSizeMB = parseInt(process.env.MAX_UPLOADS_SIZE_MB, 10) || 1024; // 1GB по умолчанию
      const currentSizeMB = Math.round(dirStats.totalSize / (1024 * 1024));
      const usagePercent = Math.round((currentSizeMB / maxSizeMB) * 100);

      if (usagePercent > 90) {
        warn(
          "[CRON:DISK_MONITOR] Критическое использование дискового пространства:",
          {
        ...
        debug("[CRON:DISK_MONITOR] Статистика дискового пространства:", {
          currentSize: `${currentSizeMB}MB`,
          maxSize: `${maxSizeMB}MB`,
          usagePercent: `${usagePercent}%`,
        });
      ...
      _error("[CRON:DISK_MONITOR] Ошибка мониторинга:", error);
    }
  }
  ...
  async getDirectoryStats(dirPath) {
    ...
    try {
      ...
      warn("[CRON] Ошибка при получении статистики директории:", error.message);
    }
   ...
   */
  logCronStats() {
    info("[CRON:STATS] Статистика выполнения:", {
      ...this.stats,
  ...
  async notifyAdminsAboutCronError(jobName, error) {
    ...
    _error(`[CRON:ALERT] Критическая ошибка в задании ${jobName}:`, {
      message: error.message,
  ...
  async runJobManually(jobName) {
    ...
    info(`[CRON] Ручной запуск задания: ${jobName}`);
   ...
   */
  stopAll() {
    info("[CRON] Остановка всех заданий");
