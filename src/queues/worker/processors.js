...
const initProcessors = async () => {
	...
	taskQueues.process("sendTelegramNotification", 5, async (job) => {
		try {
			...
			const result = await processNotification(message, level, metadata, options);
			...
			_error(`Error processing Telegram notification (Job ID: ${job.id}):`, error.message);
			...
			if (job.attemptsMade >= job.opts.attempts - 1) {
				_error(`Telegram notification ${job.id} failed after ${job.attemptsMade} attempts:`, error);
			} else {
	...
	pushNotificationsQueues.process("sendPushNotification", 10, async (job) => {
		...
		try {
			await sendPushNotificationCustom(title, body, data, options, dbSave, userId);
