...
import { logApiError, logUnexpectedError, logValidationError } from "../logger/ErrorLogger";
...
export default class ApiError extends Error {
	constructor(status, message, errors = [], requestContext = null) {
		...
		if (requestContext) {
			logApiError(this, requestContext, "API_ERROR_CREATED");
		}
	}
	...
	static BadRequest(message, errors = [], req = null) {
		...
		logApiError(error, req, "BAD_REQUEST");
		return error;
	}
	...
	static UnauthorizedError(req = null) {
		...
		logApiError(error, req, "UNAUTHORIZED");
		return error;
	}
	...
	static ForbiddenError(message = "Доступ запрещен", req = null) {
		...
		logApiError(error, req, "FORBIDDEN");
		return error;
	}
	...
	static NotFoundError(message = "Объект не найден", req = null) {
		...
		logApiError(error, req, "NOT_FOUND");
		return error;
	}
	...
	static ConflictError(message = "Конфликт данных", req = null) {
		...
		logApiError(error, req, "CONFLICT");
		return error;
	}
	...
	static TooManyRequestsError(message = "Слишком много запросов", req = null) {
		...
		logApiError(error, req, "RATE_LIMIT_EXCEEDED");
		return error;
	}
	...
	static GatewayError(message = "Ошибка шлюза", req = null) {
		...
		logApiError(error, req, "GATEWAY_ERROR");
		return error;
	}
	...
	static ServiceUnavailableError(message = "Сервис временно недоступен", req = null) {
		...
		logApiError(error, req, "SERVICE_UNAVAILABLE");
		return error;
	}
	...
	static DatabaseError(message = "Ошибка базы данных", req = null) {
		...
		logApiError(error, req, "DATABASE_ERROR");
		return error;
	}
	...
	static create(status, message, errors = [], req = null) {
		...
		logApiError(error, req, "CUSTOM_ERROR");
		return error;
	}
}
