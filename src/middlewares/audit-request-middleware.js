...
const getRequestCategory = (method, path) => {
	...
	// CREATE операции
	if (methodType === "POST" && !path.includes("/search") && !path.includes("/filter")) {
		return "CREATE";
	}
	...
	return (req, res, next) => {
		...
		if (config.logRequestBody) {
			...
			res.end = function (chunk, encoding, callback) {
				...
				// Вызываем логирование после завершения ответа
				setTimeout(
					() => logRequestCompletion(req, res, startTime, clientIp, requestId, [], config),
					0,
				);
				return end;
			};
...
const logRequestCompletion = (
	...
	try {
		...
		const logData = {
			...
			userAgent: config.includeUserAgent ? req.headers["user-agent"] : undefined,
			referer: config.includeReferer ? req.headers.referer || req.headers.referrer : undefined,
		...
		if (isError && responseBodyChunks.length > 0 && responseBodyChunks.length < 5) {
			try {
		...
		if (req.user && !isAdminPath(req.path)) {
			...
			if ((isImportant || isWriteOperation) && !isError) {
				const action = getRequestCategory(req.method, req.path);
				auditLogger.logUserEvent(req.user.id, req.user.email, "USER_REQUEST", action, {
					ip: clientIp,
					endpoint: req.path,
					method: req.method,
					statusCode,
 ...
 */
const logRequestError = (req, error, startTime, clientIp, requestId, _config) => {
	const duration = Date.now() - startTime;
