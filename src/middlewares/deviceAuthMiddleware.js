...
class DeviceAuthManager {
	...
	static createAuthMiddleware() {
		return async function deviceAuthMiddleware(req, _res, next) {
			try {
	...
	static async checkExistingUserAccess(email, deviceInfo) {
		try {
			...
			const accessCheck = DeviceAuthManager.validateDeviceAccess(user.role, deviceInfo.platform);
			...
			if (!accessCheck.allowed) {
				...
				throw ApiError.ForbiddenError(accessCheck.message, "DEVICE_ACCESS_DENIED", {
			...
			logger.info(`Login allowed for user ${user._id} (${user.role}) from ${deviceInfo.platform}`);
		} catch (error) {
	 ...
	 */
	static async checkRegistrationAccess(_email, deviceInfo) {
		// Если  - регистрируется юрист (только с веба)
		if (deviceInfo.platform !== "web") {
			const accessCheck = DeviceAuthManager.validateDeviceAccess("lawyer", deviceInfo.platform);

			if (!accessCheck.allowed) {
				logger.warn(`Lawyer registration attempt denied from platform ${deviceInfo.platform}`);
