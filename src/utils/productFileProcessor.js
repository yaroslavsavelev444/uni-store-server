 ...
 */
export async function processProductFiles(productData) {
	const result = { ...productData };

	// Обрабатываем основное изображение (если есть)
	if (result.mainImage?.url) {
	...
	if (result.images && Array.isArray(result.images)) {
		...
		for (const image of result.images) {
			...
			if (image.url) {
				const processedImage = await processSingleFile(image.url, "products/images", "gallery");
 ...
 */
const processSingleFile = async (fileUrl, targetFolder, _fileType) => {
	...
	if (fileUrl.startsWith("http://") || fileUrl.startsWith("https://")) {
		...
		if (fileUrl.startsWith(API_URL) && !cleanPath.includes("/temp/")) {
			...
			try {
				...
				console.warn(
					`Предупреждение: Отсутствует или недоступен файл существующего изображения: ${fileUrl} - ${error.message}`,
				);
			}
	...
	const originalName = basename(cleanPath);
	const extension = extname(originalName);
	const safeName = basename(originalName, extension)
 ...
 */
export async function cleanupOldProductFiles(existingProduct, newData) {
	// Удаляем старое основное изображение если оно было заменено
	if (existingProduct.mainImage?.url) {
		const shouldDeleteMainImage =
			// Новое изображение загружено
			(newData.mainImage?.url && newData.mainImage.url !== existingProduct.mainImage.url) ||
	...
	if (existingProduct.instructionFile?.url) {
		const shouldDeleteInstruction =
			// Новая инструкция загружена
			(newData.instructionFile?.url &&
