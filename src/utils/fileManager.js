...
class FileManager {
      ...
      for (const attachment of attachments) {
        ...
        try {
          ...
          movedFiles.push({
            ...
            originalName: decodedOriginalName,
            size: fileSize,
            mimeType: attachment.mimeType || FileManager.getMimeTypeFromName(decodedOriginalName),
            uploadedAt: new Date(),
            moved: true,
            movedAt: new Date(),
          });
  ...
  static getFileUrl(filePath) {
    ...
    if (filePath.startsWith("/files/")) {
      const base = process.env.NODE_ENV === "production"
        ? process.env.PUBLIC_BASE_URL || "https://api.npo-polet.ru"
        : `http://localhost:${port}`;
  ...
  static async getSignedPreviewUrl(fileId) {
