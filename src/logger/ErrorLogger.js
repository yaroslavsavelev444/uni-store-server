...
class ErrorLogger {
	...
	static logApiError(error, req, errorType = "API_ERROR") {
		try {
			...
			if (req?.user && error.status >= 400 && error.status < 500) {
				const event = ErrorLogger.mapStatusCodeToEvent(error.status);
				auditLogger.logUserEvent(req.user.id, req.user.email, event, `${errorType}_FAILED`, {
					ip: context.ip,
	...
	static logValidationError(errors, req) {
		try {
			...
			if (req?.user) {
				auditLogger.logUserEvent(req.user.id, req.user.email, "VALIDATION", "VALIDATION_FAILED", {
					ip: context.ip,
					endpoint: context.endpoint,
	...
	static getRequestContext(req) {
		...
		try {
			return {
				...
				userRole: req.user?.role ? req.user.role : "guest",
