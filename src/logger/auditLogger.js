...
class AuditLogger {
	constructor() {
		...
		this.logDir =
			process.env.NODE_ENV === "production" ? "/app/logs" : resolve(process.cwd(), "logs");
	...
	ensureDirectories() {
		const dirs = ["audit/users/current", "audit/admins/current", "application", "errors", "access"];
	...
	createSimpleLoggers() {
		...
		// Пути к файлам
		const adminLogPath = join(this.logDir, "audit/admins/current", `${today}.log`);
		const userLogPath = join(this.logDir, "audit/users/current", `${today}.log`);
