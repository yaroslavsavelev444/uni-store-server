...
const logout = async (req, res, next) => {
	try {
		...
		// Логирование выхода
		if (userData?.id) {
			await auditLogger.logUserEvent(userData.id, userData.email, "USER_AUTHENTICATION", "LOGOUT", {
				ip,
				sessionEnded: true,
				logoutType: "manual",
				tokenCleared: true,
...
const updateUser = async (req, res, next) => {
	try {
		...
		await auditLogger.logUserEvent(userId, req.user.email, "USER_PROFILE", "UPDATE_PROFILE", {
			ip,
			changes:
...
const getSessions = async (req, res, next) => {
	try {
		...
		await auditLogger.logUserEvent(userData.id, userData.email, "USER_SESSIONS", "VIEW_SESSIONS", {
			ip,
...
const revokeSession = async (req, res, next) => {
	try {
		...
		await auditLogger.logUserEvent(userData.id, userData.email, "USER_SESSIONS", "REVOKE_SESSION", {
			ip,
			sessionId,
			revokedBy: "user",
...
const initiatePasswordReset = async (req, res, next) => {
	try {
		...
		await auditLogger.logUserEvent("unknown", email, "USER_SECURITY", "PASSWORD_RESET_INITIATED", {
			ip,
			resetMethod: "email",
			emailSent: true,
		});
...
const updateOnlineStatus = async (req, res, next) => {
	try {
		...
		await auditLogger.logUserEvent(userId, req.user.email, "USER_STATUS", "UPDATE_ONLINE_STATUS", {
			ip,
			newStatus: status,
		});
