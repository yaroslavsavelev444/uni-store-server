...
class OrdersController {
  ...
  async getOrders(req, res, next) {
    try {
      ...
      const orders = await getUserOrders(req.user.id, filters);
  ...
  async getOrder(req, res, next) {
    try {
      const { id } = req.params;
      const order = await getUserOrder(id, req.user.id);
  ...
  async createOrder(req, res, next) {
    try {
      ...
      const order = await _createOrder(req.user, orderData);
  ...
  async cancelOrder(req, res, next) {
    try {
      ...
      const order = await cancelOrderByUser(id, req.user.id, reason);
      res.status(200).json({
        success: true,
  ...
  async getAdminOrders(req, res, next) {
    try {
      ...
      const result = await _getAdminOrders(filters, pagination);
  ...
  async updateOrderStatus(req, res, next) {
    try {
      ...
      const order = await _updateOrderStatus(
        id,
        status,
        req.user.id,
        comment || "",
  ...
  async cancelOrderAdmin(req, res, next) {
    try {
      ...
      const order = await cancelOrderByAdmin(
        id,
        req.user.id,
        reason,
        refundAmount,
  ...
  async uploadAttachment(req, res, next) {
    try {
      ...
      const order = await _uploadAttachment(id, filePath, userId);
  ...
  async deleteAttachment(req, res, next) {
    try {
      ...
      // Вызываем сервисный метод
      const order = await _deleteAttachment(id, fileId, userId);
