...
class OrdersController {
		...
		try {
			...
			if (deliveryMethod === DeliveryMethod.DOOR_TO_DOOR) {
				if (paymentMethod !== PaymentMethod.INVOICE) {
					throw ApiError.BadRequest("Для доставки до двери доступна только оплата по счету");
				}
	...
	async updateOrderStatus(req, res, next) {
		try {
			...
			const order = await OrderService.updateOrderStatus(id, status, req.user.id, comment || "");
	...
	async cancelOrderAdmin(req, res, next) {
		try {
			...
			const order = await OrderService.cancelOrderByAdmin(id, req.user.id, reason, refundAmount);
