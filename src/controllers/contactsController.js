...
class СontactsController {
  ...
  async getContacts(req, res, next) {
    try {
      ...
      if (!req) {
        ...
        return res.json({
          success: true,
          data: getEmptyStructureForUsers(),
      ...
      const isAdmin =
        (req.user && req.user.role && req.user.role.includes("admin")) || false;

      // Для обычных пользователей всегда получаем только активные контакты
      const contacts = await _getContacts(isAdmin);
  ...
  async updateContacts(req, res, next) {
    try {
      ...
      const contacts = await _updateContacts(req.body, userId);
  ...
  async toggleActive(req, res, next) {
    try {
      ...
      const isActive = await _toggleActive(userId);
  ...
  async getChangeHistory(req, res, next) {
    try {
      ...
      const history = await _getChangeHistory(parseInt(limit));
  ...
  async exportVCard(req, res, next) {
    try {
      ...
      const vCard = await exportAsVCard(isAdmin);
      ...
      const emptyVCard = generateEmptyVCard();
      res.set({
        "Content-Type": "text/vcard",
        "Content-Disposition": 'attachment; filename="contacts.vcf"',
...
export default new СontactsController();
