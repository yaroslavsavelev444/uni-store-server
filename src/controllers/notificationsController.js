...
const markNotificationAsRead = async (req, res, next) => {
	...
	try {
		const notifications = await notificationsService.markNotificationAsReadService(ids, userData);
		logger.info("notifications", notifications);
		res.status(200).json(notifications);
	} catch (e) {
		next(e);
...
const deleteNotifications = async (req, res, next) => {
	...
	try {
		const notifications = await notificationsService.deleteNotificationsService(userData);
		res.status(200).json(notifications);
	} catch (e) {
		next(e);
	}
