...
import { UserModel, UserSecurityModel, UserSessionModel } from "../models/index.models";
...
const verify2FACodeOnly = async (userId, inputCode) => {
	try {
		...
		if (!isValid) {
			...
			await UserSecurityModel.updateOne({ userId }, { $inc: { twoFAAttempts: 1 } });
...
async function createOrUpdateSession({
	...
	const filter = deviceId ? { userId, deviceId } : { userId, deviceType, deviceModel, ip };
...
async function pruneSessions(userId, maxSessions) {
	...
	if (sessions?.length) {
		...
		await UserSessionModel.updateMany({ _id: { $in: ids } }, { $set: { revoked: true } });
		// логируем
