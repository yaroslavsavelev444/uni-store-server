...
if (!KEK) {
	...
	warn("KEY_ENCRYPTION_KEY not set — encryption operations will fail until it is set.");
}
...
function wrapDEK(kekBuf, dekBuf) {
	...
	const cipher = createCipheriv(ALGO, kekBuf, iv, { authTagLength: 16 });
...
function unwrapDEK(kekBuf, wrappedBase64) {
	...
	const decipher = createDecipheriv(ALGO, kekBuf, iv, { authTagLength: 16 });
...
function encryptWithDEK(dekBuf, plaintext, aad = null) {
	const iv = randomBytes(IV_LEN);
	const cipher = createCipheriv(ALGO, dekBuf, iv, { authTagLength: 16 });
...
function decryptWithDEK(dekBuf, payloadBase64, aad = null) {
	...
	const tag = data.slice(IV_LEN, IV_LEN + 16);
	const ct = data.slice(IV_LEN + 16);
	const decipher = createDecipheriv(ALGO, dekBuf, iv, { authTagLength: 16 });
