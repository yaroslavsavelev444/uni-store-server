...
class CartService {
	async getCart(userId) {
		...
		const totalPrice = 0;
		const totalPriceWithoutDiscount = 0;
		let itemsToUpdate = false;
		const validationIssues = [];
		const isCartValid = true;
    ...
    cart.items = processedItems.map(i => (
					product: i.product._id, quantity;
					: i.quantity, addedAt: i.addedAt
					))
					await cart.save();
				}
      ...
      cart.items = processedItems.map((i) => (
		...
		async;
		addOrUpdateItem(userId, productId, quantity);
		{
		 ...
		 */
		async;
		calculateCartDiscounts(cartData);
