...
class BannerService {
    ...
    for (const mediaUrl of uploadedMedia) {
      ...
      try {
        ...
        if (mediaUrl.includes("/temp/")) {
          ...
          // Перемещаем файл
          const movedPath = await moveFile(tempPath, permanentPath);
          ...
          try {
            await validateFileExists(mediaUrl);
  ...
  async createBanner({ bannerData, uploadedMedia, userId }) {
    ...
    if (
      bannerData.endAt &&
      bannerData.endAt !== "null" &&
      bannerData.endAt !== "undefined"
    ) {
  ...
  async updateBanner({ id, bannerData, uploadedMedia, deletedUrls, userId }) {
    ...
    if (!banner) throw NotFoundError("Баннер не найден");
    ...
    if (deletedUrls && deletedUrls.length > 0) {
      ...
      banner.media = banner.media.filter(
        (mediaUrl) => !deletedUrls.includes(mediaUrl),
      );
      console.log(
    ...
    if (bannerData.actionPayload !== undefined)
      banner.actionPayload = bannerData.actionPayload;
  ...
  async getBannerForUser(user) {
    ...
    for (const banner of banners) {
      ...
      if (!banner.repeatable) {
        ...
        const wasShown = await get(bannerKey);
        ...
        const lastShown = await get(repeatKey);
    ...
    await set(cooldownKey, "1", "EX", COOLDOWN_TTL);

    // Фиксируем показ баннера
    const bannerKey = `banner:shown:${user.id}:${selectedBanner._id}`;
    await set(bannerKey, "1", "EX", 60 * 60 * 24 * 7); // 7 дней
    ...
    if (selectedBanner.repeatable) {
      ...
      await set(repeatKey, "1", "EX", 60 * 60 * 24); // 24 часа
    }
  ...
  async changeStatus(id, status) {
    if (!["draft", "active", "archived"].includes(status)) {
      throw BadRequest("Неверный статус");
    }

    const banner = await BannerModel.findById(id);
    if (!banner) {
      throw NotFoundError("Баннер не найден");
    }
