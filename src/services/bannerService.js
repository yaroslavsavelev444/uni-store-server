...
class BannerService {
	...
	async createBanner({ bannerData, uploadedMedia, userId }) {
		...
		const banner = new BannerModel({
			...
			targeting: {
				roles: bannerData.targeting?.roles ? this.parseMaybeArray(bannerData.targeting.roles) : [],
			},
	...
	async getBannerForUser(user) {
		...
		if (user.role) {
			...
			query.$or = [{ "targeting.roles": { $exists: false } }, { "targeting.roles": { $size: 0 } }];
		}

		// Получаем баннеры, сортируем по приоритету
		const banners = await BannerModel.find(query).sort({ priority: -1, createdAt: -1 }).lean();
