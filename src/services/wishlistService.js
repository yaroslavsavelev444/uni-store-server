...
class WishlistService {
  ...
  async addProduct(userId, productId, notes = "") {
    if (!userId || !productId) {
      throw BadRequest("ID пользователя и товара обязательны");
    }
    ...
    if (!product.isVisible) {
      throw BadRequest("Товар недоступен для добавления в избранное");
    }
  ...
  async removeProduct(userId, productId) {
    ...
    if (!wishlist) {
      throw NotFoundError("Избранное не найдено");
    }
    ...
    if (wishlist.items.length === initialLength) {
      throw NotFoundError("Товар не найден в избранном");
    }
