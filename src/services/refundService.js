...
class RefundService {
  ...
  async createRefund(refundData, userId) {
    try {
      ...
      throw DatabaseError("Ошибка при создании заявки на возврат");
    }
  }
  ...
  async getUserRefunds(userId, query = {}) {
    ...
    try {
      ...
      throw DatabaseError("Ошибка при получении заявок на возврат");
    }
  }
  ...
  async getRefundById(id, userId, isAdmin = false) {
    try {
      ...
      if (!refund) {
        throw NotFound("Заявка на возврат не найдена");
      }

      // Проверка прав доступа
      if (!isAdmin && refund.userId.toString() !== userId.toString()) {
        throw Forbidden("У вас нет доступа к этой заявке");
      }

      return this.formatRefundForResponse(refund);
    } catch (error) {
      if (error instanceof ApiError) throw error;
      throw DatabaseError("Ошибка при получении заявки на возврат");
    }
  }
  ...
  async updateRefundStatus(id, statusData, adminId, adminName) {
    if (!Types.ObjectId.isValid(id)) {
      throw BadRequest("Некорректный формат ID заявки");
    }

    if (!Object.values(RefundStatus).includes(statusData.status)) {
      throw BadRequest("Некорректный статус заявки");
    }
    ...
    try {
      ...
      if (error instanceof ApiError) throw error;
      throw DatabaseError("Ошибка при обновлении статуса заявки");
    }
  }
  ...
  async assignRefundToAdmin(id, adminId, adminName, assignerId) {
    ...
    try {
      ...
      if (error instanceof ApiError) throw error;
      throw DatabaseError("Ошибка при назначении заявки");
    }
  }
  ...
  async addAdminNote(id, noteData, adminId, adminName) {
    ...
    try {
      ...
      if (error instanceof ApiError) throw error;
      throw DatabaseError("Ошибка при добавлении заметки");
    }
  }
  ...
  async getRefundStats(timeframe = "month") {
    ...
    try {
      ...
      const result = {
        timeframe,
        total:
          stats[0]?.statusCounts?.reduce((sum, item) => sum + item.count, 0) ||
          0,
        totalAmount:
      ...
      throw DatabaseError("Ошибка при получении статистики");
    }
  }
  ...
  validateRefundData(data) {
    if (!data.orderId || !Types.ObjectId.isValid(data.orderId)) {
      throw BadRequest("Некорректный ID заказа");
    }

    if (!data.items || !Array.isArray(data.items) || data.items.length === 0) {
      throw BadRequest("Не указаны товары для возврата");
    }

    if (!data.description || data.description.length < 10) {
      throw BadRequest("Описание должно содержать минимум 10 символов");
    }

    if (!data.reason || !Object.values(RefundReason).includes(data.reason)) {
      throw BadRequest("Некорректная причина возврата");
    }
    ...
    if (data.media && Array.isArray(data.media)) {
      data.media.forEach((media) => {
        if (!media.url || typeof media.url !== "string") {
          throw BadRequest("Некорректный формат медиафайла");
        }
      });
    }
  }
