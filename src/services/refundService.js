...
class RefundService {
	...
	async createRefund(refundData, userId) {
		try {
			...
			const activeRefund = await RefundModel.findOne({
				...
				status: {
					$in: [RefundStatus.PENDING, RefundStatus.PROCESSING, RefundStatus.APPROVED],
				},
			});
	...
	async getUserRefunds(userId, query = {}) {
		...
		try {
			...
			const { status, limit = 10, page = 1, sortBy = "createdAt", sortOrder = "desc" } = query;
	...
	async updateRefundStatus(id, statusData, adminId, _adminName) {
	...
	async getRefundReasons() {
		return Object.entries(RefundReason).map(([_key, value]) => ({
			key: value,
	...
	async getRefundStatuses() {
		return Object.entries(RefundStatus).map(([_key, value]) => ({
			key: value,
