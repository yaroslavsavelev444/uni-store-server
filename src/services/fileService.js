...
class FileService {
	constructor() {
		this.uploadsDir = join(process.cwd(), uploadsDir);
	...
	async ensureDirectories() {
		try {
			...
			await Promise.all([
				fs.writeFile(join(this.uploadsDir, ".gitkeep"), ""),
				fs.writeFile(join(this.tempDir, ".gitkeep"), ""),
			]);
	...
	async saveFileMetadata(userId, fileMeta) {
		const metaPath = join(this.tempDir, `${userId}_meta.json`);
		let existingMeta = [];
	...
	async deleteFiles(tempNames) {
		...
		for (const tempName of tempNames) {
			try {
				...
				const filePath = join(this.tempDir, tempName);
	...
	async deleteFilesByUrls(fileUrls) {
		...
		for (const fileUrl of fileUrls) {
			try {
				...
				const _success = await FileManager.deleteFile(fileUrl);
	...
	async cleanupMetadata(tempName) {
		try {
			...
			for (const metaFile of userMetaFiles) {
				const metaPath = join(this.tempDir, metaFile);
				const metaContent = await fs.readFile(metaPath, "utf8");
	...
	async getUserFiles(userId) {
		try {
			...
			for (const fileMeta of metadata) {
				const filePath = join(this.tempDir, fileMeta.tempName);
