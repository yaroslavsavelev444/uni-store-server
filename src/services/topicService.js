...
class TopicService {
	...
	async getAll() {
		...
		const items = await TopicModelCommon.find({}).sort({ position: 1, createdAt: -1 }).lean();

		// Кешируем результат
		try {
			await this.redisClient.setJson(cacheKey, items, this.CACHE_TTL.ALL_TOPICS);
	...
	async getItemById(id) {
		...
		if (item) {
			try {
				await this.redisClient.setJson(cacheKey, item, this.CACHE_TTL.SINGLE_TOPIC);
	...
	async getBySlugWithRelated(slug) {
		...
		// Кешируем результат
		try {
			await this.redisClient.setJson(cacheKey, result, this.CACHE_TTL.SINGLE_TOPIC);
	...
	async create(data, files) {
		...
		const imageUrl = coverFile ? join(baseUrl, coverFile.filename).replace(/\\/g, "/") : "";
		...
		contentImages.forEach((file) => {
			...
			fileMap[originalBaseName] = join(baseUrl, file.filename).replace(/\\/g, "/");
		});
	...
	async update(id, data) {
		...
		// Инвалидируем кеш
		await this.invalidateCache("topics:all", `topics:slug:*`, `topics:id:${id}`, "topics:related");
