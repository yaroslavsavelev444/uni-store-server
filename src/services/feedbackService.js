...
class FeedbackService {
  ...
  async submitFeedback(data) {
    ...
    try {
      ...
      if (!title?.trim() || !description?.trim() || !type) {
        throw BadRequest("Заголовок, описание и тип обязательны");
      }
      ...
      if (attachments && attachments.length > 0) {
        ...
        const normalizedAttachments = attachments.map((att) => ({
          ...
          originalName: att.originalName
            ? normalizeFileName(att.originalName)
            : att.tempName,
        }));

        movedAttachments = await moveTempFilesToPermanent(
      ...
      _error("[FEEDBACK_SUBMIT] Ошибка:", error);
      if (error instanceof ApiError) throw error;
      throw InternalServerError("Ошибка при создании фидбека");
    }
  }
  ...
  async cleanupUserTempFiles(userId) {
    try {
      ...
      if (allTempNames.length > 0) {
        await cleanupTempFiles(allTempNames);
        console.log(
  ...
  async getFeedback(id, userId, userRole) {
    try {
      ...
      if (!feedback) {
        throw NotFoundError("Фидбек не найден");
      }
      ...
      if (!isOwner && !isAdmin) {
        throw ForbiddenError("Нет доступа к этому фидбеку");
      }
      ...
      if (error instanceof ApiError) throw error;
      _error("[GET_FEEDBACK] Ошибка:", error);
      throw InternalServerError("Ошибка при получении фидбека");
    }
  }
  ...
  async getAllFeedbacks(options = {}) {
    try {
      ...
      _error("[GET_ALL_FEEDBACKS] Ошибка:", error);
      throw InternalServerError("Ошибка при получении фидбеков");
    }
  }
  ...
  async updateStatus(feedbackId, status, adminId, note) {
    ...
    try {
      ...
      if (error instanceof ApiError) throw error;
      _error("[UPDATE_STATUS] Ошибка:", error);
      throw InternalServerError("Ошибка при обновлении статуса");
    } finally {
      session.endSession();
    }
  }
  ...
  async updatePriority(feedbackId, priority, adminId) {
    try {
      ...
      if (error instanceof ApiError) throw error;
      _error("[UPDATE_PRIORITY] Ошибка:", error);
      throw InternalServerError("Ошибка при обновлении приоритета");
    }
  }
  ...
  async addInternalNote(feedbackId, note, adminId, isPrivate = false) {
    try {
      ...
      if (error instanceof ApiError) throw error;
      _error("[ADD_INTERNAL_NOTE] Ошибка:", error);
      throw InternalServerError("Ошибка при добавлении заметки");
    }
  }
  ...
  async updateInternalNote(feedbackId, noteId, note, adminId) {
    try {
      ...
      if (error instanceof ApiError) throw error;
      _error("[UPDATE_INTERNAL_NOTE] Ошибка:", error);
      throw InternalServerError("Ошибка при обновлении заметки");
    }
  }
  ...
  async deleteInternalNote(feedbackId, noteId, adminId) {
    try {
      ...
      if (!feedback) {
        throw NotFoundError("Заметка не найдена или нет прав для удаления");
      }

      return feedback;
    } catch (error) {
      if (error instanceof ApiError) throw error;
      _error("[DELETE_INTERNAL_NOTE] Ошибка:", error);
      throw InternalServerError("Ошибка при удалении заметки");
    }
  }
  ...
  async addTag(feedbackId, tag, adminId) {
    try {
      ...
      if (error instanceof ApiError) throw error;
      _error("[ADD_TAG] Ошибка:", error);
      throw InternalServerError("Ошибка при добавлении тега");
    }
  }
  ...
  async removeTag(feedbackId, tag, adminId) {
    try {
      ...
      if (error instanceof ApiError) throw error;
      _error("[REMOVE_TAG] Ошибка:", error);
      throw InternalServerError("Ошибка при удалении тега");
    }
  }
  ...
  async markAsDuplicate(feedbackId, duplicateOfId, adminId, note) {
    ...
    try {
      ...
      if (!originalFeedback) {
        throw NotFoundError("Оригинальный фидбек не найден");
      }
      ...
      if (feedbackId === duplicateOfId) {
        throw BadRequest("Нельзя пометить фидбек как дубликат самого себя");
      }
      ...
      await session.abortTransaction();
      if (error instanceof ApiError) throw error;
      _error("[MARK_AS_DUPLICATE] Ошибка:", error);
      throw InternalServerError("Ошибка при помечении как дубликат");
    } finally {
      session.endSession();
    }
  }
  ...
  async deleteFeedback(feedbackId, adminId) {
    try {
      ...
      if (error instanceof ApiError) throw error;
      _error("[DELETE_FEEDBACK] Ошибка:", error);
      throw InternalServerError("Ошибка при удалении фидбека");
    }
  }
  ...
  async incrementViewCount(feedbackId) {
    try {
      ...
      _error("[INCREMENT_VIEW_COUNT] Ошибка:", error);
    }
  }
  ...
  async getAdminStats() {
    try {
      ...
      _error("[GET_ADMIN_STATS] Ошибка:", error);
      throw InternalServerError("Ошибка при получении статистики");
    }
  }
  ...
  async getUserStats(userId) {
    try {
      ...
      _error("[GET_USER_STATS] Ошибка:", error);
      throw InternalServerError("Ошибка при получении статистики пользователя");
    }
  }
  ...
  async exportToCSV(options = {}) {
    try {
      ...
      _error("[EXPORT_TO_CSV] Ошибка:", error);
      throw InternalServerError("Ошибка при экспорте данных");
    }
  }
  ...
  async notifyAdminsAboutNewFeedback(feedback) {
    try {
      ...
      info("Уведомления администраторам отправлены", {
        feedbackId: feedback._id,
        adminCount: admins.length,
      });
    } catch (error) {
      _error("[NOTIFY_ADMINS] Ошибка:", error);
  ...
  async notifyUserAboutStatusChange(feedback, oldStatus, newStatus) {
    try {
      ...
      _error("[NOTIFY_USER_STATUS_CHANGE] Ошибка:", error);
    }
  }
  ...
  async notifyAssignedUser(feedback, assignedUser) {
    try {
      ...
      _error("[NOTIFY_ASSIGNED_USER] Ошибка:", error);
    }
  }
}
