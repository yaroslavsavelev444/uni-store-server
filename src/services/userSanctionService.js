...
class UserSanctionService {
	...
	async blockUser(userId, admin, options = {}) {
		...
		try {
			...
			throw ApiError.DatabaseError(`Ошибка при блокировке пользователя: ${error.message}`, null);
		} finally {
			session.endSession();
		}
	}
	...
	async unblockUser(userId, admin) {
		...
		try {
			...
			throw ApiError.DatabaseError(`Ошибка при разблокировке пользователя: ${error.message}`, null);
		} finally {
			session.endSession();
		}
	}
	...
	async getUserSanctions(userId) {
		try {
			...
			throw ApiError.DatabaseError(`Ошибка при получении истории санкций: ${error.message}`, null);
		}
	}
	...
	async getActiveSanctions(userId) {
		try {
			...
			throw ApiError.DatabaseError(`Ошибка при получении активных санкций: ${error.message}`, null);
		}
	}
	...
	async checkUserBlockStatus(userId) {
		try {
			const user = await UserModel.findById(userId).select("status blockedUntil name email").lean();
