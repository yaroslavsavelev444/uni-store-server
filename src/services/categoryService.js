...
class CategoryService {
  ...
  async getAllCategories(filters = {}, options = {}) {
    ...
    // Выполняем запрос
    const categories = await CategoryModel.find(query).sort(sortOptions).lean();
  ...
  async getCategoryById(id, options = {}) {
    if (!Types.ObjectId.isValid(id)) {
      throw BadRequest("Некорректный формат ID категории");
    }
    ...
    if (!category.isActive && !options.includeInactive) {
      throw NotFound("Категория не доступна");
    }
  ...
  async createCategory(categoryData, userId) {
    ...
    if (categoryData.slug) {
      ...
      if (existingCategory) {
        throw BadRequest("Категория с таким slug уже существует");
      }
    }
    ...
    if (updateData.image !== undefined) {
      if (updateData.image === null) {
        if (existingCategory.image?.url) {
          await deleteFile(existingCategory.image.url);
        }
