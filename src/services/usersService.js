...
class UserService {
	 ...
	 */
	async getAllUsers(_currentUser) {
		try {
	...
	async updateUserRole(userId, newRole, currentUser) {
		try {
			...
			if (userId.toString() === currentUser.id.toString()) {
				throw ApiError.BadRequest("Вы не можете изменить свою собственную роль", [], null);
			}
			...
			if (currentUser.role !== "superadmin" && newRole === "superadmin") {
				throw ApiError.ForbiddenError("Только суперадмин может назначать роль суперадмина", null);
			}
...
`,
			...
			throw ApiError.DatabaseError(`Ошибка при получении пользователя: ${error.message}`, null);
		}
	}
...
`,
			...
			throw ApiError.DatabaseError(`Ошибка при поиске пользователей: ${error.message}`, null);
		}
	}
