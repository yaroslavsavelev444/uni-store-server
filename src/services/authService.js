...
import _default from "./2faService";

const { create2FACodeAndNotify, verify2FACode, verify2FACodeOnly } = _default;

import UserDTO from "../dtos/user.dto";
import __default from "./tokenService";
...
import ___default from "../queues/taskQueues";
import moveFileToFinal from "../utils/moveFileToFinal";

const { sendEmailNotification, sendPushNotification } = ___default;
...
const register = async (userData, meta = {}) => {
	try {
		...
		throw ApiError.InternalServerError("Произошла ошибка при регистрации", error);
	}
};

//VERIFIERS
const verify2FAAndNotify = async (userId, inputCode, deviceType, ip, device) => {
	try {
		const result = await verify2FACode(userId, inputCode, deviceType, ip, device);
...
const refreshService = async (refreshToken, _deviceType, ip) => {
	if (!refreshToken) {
...
const updateUser = async (userId, userData, files) => {
	try {
		...
		if (files?.avatar?.[0]) {
			...
			const finalFilePath = path.join(__dirname, "..", "uploads", "users", uploadedFile.filename);

			await moveFileToFinal(tempFilePath, finalFilePath);

			// Добавляем в payload для обновления
			updatePayload.avatarUrl = path.join("uploads", "users", uploadedFile.filename);
		}
...
const checkService = async (accessToken, refreshToken, deviceType, ip) => {
	console.log("checkService called", !!accessToken, !!refreshToken, deviceType, ip);
	...
	const { accessToken: newAccess } = generateToken({ ...userDto }, { onlyAccess: true });
...
const resendResetCode = async (email) => {
	try {
		...
		if (userSecurity?.resetTokenExpiration) {
			const timeSinceLastRequest = Date.now() - userSecurity.resetTokenExpiration.getTime();
