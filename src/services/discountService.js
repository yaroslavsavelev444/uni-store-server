...
class DiscountService {
	...
	async createDiscount({ discountData, userId }) {
		try {
			...
			if (!discountData.isUnlimited && discountData.endAt) {
				const startAt = discountData.startAt ? new Date(discountData.startAt) : new Date();
				const endAt = new Date(discountData.endAt);

				if (endAt <= startAt) {
					throw ApiError.BadRequest("Дата окончания должна быть позже даты начала");
				}
			}
	...
	async updateDiscount({ id, discountData, userId }) {
		try {
			...
			// Проверяем даты
			if (discountData.endAt || discountData.startAt || discountData.isUnlimited !== undefined) {
				const isUnlimited =
					discountData.isUnlimited !== undefined ? discountData.isUnlimited : discount.isUnlimited;

				if (!isUnlimited) {
					const startAt = discountData.startAt ? new Date(discountData.startAt) : discount.startAt;
					const endAt = discountData.endAt ? new Date(discountData.endAt) : discount.endAt;
	...
	async listDiscounts(filters = {}) {
		try {
			...
			console.error("[DISCOUNT_SERVICE] Ошибка получения списка скидок:", error);
			throw error;
		}
	}
	...
	async changeDiscountStatus(id, isActive) {
		try {
			...
			console.error("[DISCOUNT_SERVICE] Ошибка изменения статуса скидки:", error);
			throw error;
		}
	}
	...
	async incrementDiscountUsage(discountId, discountAmount) {
		try {
			if (!discountId || !Types.ObjectId.isValid(discountId)) {
				console.warn(`[DISCOUNT_SERVICE] Некорректный ID скидки: ${discountId}`);
				return;
			}
			...
			console.error("[DISCOUNT_SERVICE] Ошибка увеличения счетчика использования:", error);
