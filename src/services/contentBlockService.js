...
import { ContentBlockModel } from "../models/contentBlock.model";
...
class ContentBlockService {
	...
	async getAll(includeInactive = false) {
		const cacheKey = includeInactive ? this.CACHE_KEYS.ALL_BLOCKS : this.CACHE_KEYS.ACTIVE_BLOCKS;
		...
		const items = await ContentBlockModel.find(query).sort({ position: 1, createdAt: -1 }).lean();
	...
	async create(contentBlockData, userId) {
		...
		try {
			...
			if (contentBlockData.image?.url) {
				contentBlockData.imageUrl = await this.processImage(contentBlockData.imageUrl);
			}
