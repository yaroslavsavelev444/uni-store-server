...
import { OrderModel, OrderStatus } from "../models/index.models";
	...
	static async hasUserPurchasedProduct(userId, productId) {
		...
		try {
			const completedOrders = await OrderModel.exists({
				user: userId,
				status: {
					$in: [OrderStatus.DELIVERED, OrderStatus.READY_FOR_PICKUP, OrderStatus.SHIPPED],
				},
			});
	...
	static async hasUserPurchasedProducts(userId, productIds) {
		...
		try {
			...
			const purchasePromises = productIds.map(async (productId) => {
				const hasPurchased = await PurchaseCheckService.hasUserPurchasedProduct(userId, productId);
