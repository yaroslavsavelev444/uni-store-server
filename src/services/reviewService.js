...
class ReviewsService {
	...
	async createReview(data) {
		...
		sendEmailNotification(process.env.ADMIN_EMAIL || "admin@example.com", "newProductReview", {
			reviewId: review._id,
	...
	async updateReviewStatus(reviewId, status) {
		const review = await ProductReviewModel.findById(reviewId).populate("product", "_id");
		...
		if (oldStatus !== status) {
			if (status === "approved") {
				...
				await this.ratingService.updateProductRating(review.product._id, review.rating);
			} else if (oldStatus === "approved" && status === "rejected") {
				// Удаляем рейтинг при отклонении ранее одобренного отзыва
				await this.ratingService.updateProductRating(review.product._id, null, review.rating, true);
			...
			try {
				await redisClient.del(`${this.PRODUCT_STATS_CACHE_PREFIX}${review.product._id}`);
			} catch (error) {
	...
	async invalidateCache(productId, userId) {
		try {
			await redisClient.deletePattern(`${this.PRODUCT_REVIEWS_CACHE_PREFIX}${productId}:*`);

			await redisClient.del(`${this.PRODUCT_STATS_CACHE_PREFIX}${productId}`);

			if (userId) {
				await redisClient.deletePattern(`${this.USER_REVIEWS_CACHE_PREFIX}${userId}:*`);
			}
		} catch (error) {
