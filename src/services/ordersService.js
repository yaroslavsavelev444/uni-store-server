...
import { sendEmailNotification, sendPushNotification } from "../queues/taskQueues";
import fileService from "../utils/fileManager";
...
class OrderService {
	...
	async createOrder(user, orderData) {
		...
		try {
					...
					try {
						...
						for (const field of requiredFields) {
							if (!orderData.newCompanyData[field]) {
								throw ApiError.BadRequest(`Для создания компании укажите ${field}`);
							}
						}
						...
						createdCompany = await CompanyService.createCompany(user.id, {
							...
							legalAddress:
								orderData.newCompanyData.legalAddress || orderData.newCompanyData.companyAddress,
						...
						logger.info(`[OrderService] Создана новая компания ${createdCompany._id} для заказа`);
					} catch (error) {
						if (error instanceof ApiError && error.message.includes("ИНН уже существует")) {
							// Пробуем найти существующую компанию с таким ИНН
							try {
								const existingCompany = await CompanyService.getCompanyByTaxNumber(
									user.id,
			...
			for (const discount of cart.discounts.applied) {
				try {
					await this.discountService.incrementDiscountUsage(discount._id, discount.amount);
					logger.info(
	 ...
	 */
	calculateShippingCost(deliveryMethod, _deliveryData, items) {
		...
		try {
			const order = await OrderModel.findOne({
				...
				.populate({
					path: "delivery.pickupPoint",
					model: "PickupPoint",
					select: "name address coordinates workingHours contact description isActive",
				})
	...
	async cancelOrderByUser(orderId, userId, reason) {
		...
		try {
			...
			await sendEmailNotification(process.env.SMTP_USER, "orderCancelledByUser", {
	 ...
	 * Расчет налога
	 */
	calculateTax(_subtotal, _orderData) {
	...
	async sendShippingNotification(order) {
		try {
			const populatedOrder = await OrderModel.findById(order._id).populate("user", "email name");
	...
	async sendPickupReadyNotification(order) {
		try {
			...
			if (populatedOrder?.user) {
				await sendEmailNotification(populatedOrder.user.email, "orderReadyForPickup", {
					orderNumber: order.orderNumber,
	...
	async uploadAttachment(orderId, filePath, userId) {
		...
		if (filePath.startsWith("http://") || filePath.startsWith("https://")) {
			...
			console.log(`[OrderService] Извлечен и декодирован путь из URL: ${cleanPath}`);
		}
		...
		if (cleanPath.includes("/temp/")) {
			const newPath = await this.moveAttachmentFromTemp(cleanPath, order.orderNumber);
			cleanPath = newPath;
		} else {
		...
		const fileInfo = await fileService.getFileInfo(fileService.getAbsolutePath(cleanPath));
	...
	async moveAttachmentFromTemp(tempPath, orderNumber) {
		console.log(`[OrderService] moveAttachmentFromTemp вызван с путем: ${tempPath}`);
		...
		if (!cleanPath.includes("/temp/")) {
			console.log(`[OrderService] Путь не из temp, возвращаем как есть: ${cleanPath}`);
		...
		try {
			...
			console.error(`[OrderService] Исходный файл не найден: ${sourceAbsolute}`, error);
		...
		try {
			...
			try {
				...
				console.error(`[OrderService] Ошибка при копировании файла:`, copyError);
	 ...
	 */
	async deleteAttachment(orderId, fileId, _userId) {
		console.log(`[OrderService] Удаление вложения ${fileId} из заказа ${orderId}`);
		...
		// Находим вложение
		const attachmentIndex = order.attachments.findIndex((a) => a._id.toString() === fileId);
		...
		if (attachment.path) {
			try {
				...
				console.log(`[OrderService] Физический файл удален: ${attachment.path}`);
			} catch (error) {
				console.warn(`[OrderService] Не удалось удалить физический файл: ${error.message}`);
	...
	async sendOrderCancelledNotification(order, reason, refundAmount) {
		try {
			...
			if (populatedOrder?.user) {
				await sendEmailNotification(populatedOrder.user.email, "orderCancelledByAdmin", {
					orderNumber: order.orderNumber,
					reason,
					refundAmount,
					orderData: order.toObject(),
