		...
		media: [
			{
				url: {
					...
					required: true,
					validate: {
						validator: (v) => /^(https?:\/\/|\/uploads\/)/.test(v),
...
RefundSchema.virtual("isOverdue").get(function () {
	...
	return new Date() > this.dueDate && ["pending", "processing"].includes(this.status);
});
...
RefundSchema.methods.assignToAdmin = function (adminId, adminName) {
	...
	this.addAdminNote(`Заявка назначена администратору: ${adminName}`, adminId, "Система");
	return this.save();
};

export default model("Refund", RefundSchema);
const _RefundStatus = RefundStatus;

export { _RefundStatus as RefundStatus };

const _RefundReason = RefundReason;

export { _RefundReason as RefundReason };
