...
const createProductSchema = object({
	...
	priceForIndividual: number().required().positive().precision(2).max(100000000),
	...
	maxOrderQuantity: number().integer().min(ref("minOrderQuantity")).max(10000).optional(),
...
const productQuerySchema = object({
	...
	sortBy: string().valid("price", "title", "createdAt", "updatedAt", "popularity"),
	...
	showOnMainPage: boolean().default(false),
	populate: string().valid("category", "relatedProducts", "all", "none").default("none"),
...
const validateProduct = (schema) => {
	return (req, res, next) => {
		...
		const { error, value } = validateProductWithLogging(schema, data, `${req.method} ${req.path}`);
