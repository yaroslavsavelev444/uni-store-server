...
// Функция не используется, поэтому удаляем (была getDeliveryAddressSchema)
  ...
  taxNumber: Joi.string()
    ...
    .custom((value, helpers) => {
      ...
      if (cleaned.length === 10) {
        ...
        for (let i = 0; i < 9; i++) {
          sum += parseInt(cleaned[i], 10) * weights[i]; // добавлен radix 10
        }
      ...
      if (cleaned.length === 12) {
        ...
        for (let i = 0; i < 10; i++) {
          sum11 += parseInt(cleaned[i], 10) * weights11[i]; // radix 10
        }

        // Вторая контрольная цифра (12-я в номере)
        for (let i = 0; i < 11; i++) {
          sum12 += parseInt(cleaned[i], 10) * weights12[i]; // radix 10
        }
        ...
        if (
          parseInt(cleaned[10], 10) !== controlNumber11 || // radix 10
          parseInt(cleaned[11], 10) !== controlNumber12 // radix 10
        ) {
          return helpers.message("Неверный ИНН (неверная контрольная сумма)");
        }
      }
    ...
    .conditional("deliveryMethod", [
      {
        ...
        // biome-ignore lint/suspicious/noThenProperty: part of Joi conditional schema
