...
const submitFeedbackSchema = object({
	title: string().required().min(5).max(200).custom(sanitizeInput, "Sanitization").messages({
	...
	type: string().valid("bug", "improvement", "feature", "other").required().messages({
...
const prioritySchema = object({
	priority: string().valid("low", "medium", "high", "critical").required().messages({
...
const addNoteSchema = object({
	note: string().required().min(1).max(1000).custom(sanitizeInput, "Sanitization").messages({
...
const paginationSchema = object({
	...
	search: string().max(100).optional().custom(sanitizeInput, "Sanitization").messages({
