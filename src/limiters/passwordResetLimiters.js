...
import { createIpRateLimiter, createRedisRateLimiter } from "../middlewares/rateLimit";
...
const initiatePasswordResetLimiter = createRedisRateLimiter({
	...
	getMax: (_req) => 3,
	getKey: (req) =>
		req.body.email ? `email:${req.body.email.toLowerCase().trim()}` : `ip:${req.ip}`,
...
const resendPasswordResetCodeLimiter = createRedisRateLimiter({
	...
	getKey: (req) =>
		req.body.email ? `email:${req.body.email.toLowerCase().trim()}` : `ip:${req.ip}`,
	message: "Слишком много запросов на повторную отправку кода. Попробуйте через 5 минут.",
});
