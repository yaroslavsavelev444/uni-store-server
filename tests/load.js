...
export default function () {
  ...
  group("Auth Scenario", () => {
    const payload = JSON.stringify({ username: "testuser", password: "pass" });
    const params = { headers: { "Content-Type": "application/json" } };
    const res = http.post(`${BASE_URL}/login`, payload, params); // Замени на реальный эндпоинт
  ...
  group("User Actions", () => {
    // GET на /api/users
    const res1 = http.get(`${BASE_URL}/products`);
    check(res1, {
      "get products success": (r) => r.status === 200 && r.json().length > 0,
    });
    ...
    const res2 = http.get(`${BASE_URL}/consents`, postPayload, {
      headers: { "Content-Type": "application/json" },
    });
    check(res2, { "create post success": (r) => r.status === 201 });

    // GET на /api/search с параметрами
    const searchParams = { tags: ["param1=value1", "param2=value2"] };
    const res3 = http.get(`${BASE_URL}/categories`, { params: searchParams });
